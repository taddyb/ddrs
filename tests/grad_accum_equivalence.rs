...
fn check_equivalence(loss_cfg: &LossConfig) -> (f32, f32, f32) {
    ...
    let pooled_loss = batch_loss(p_all, o.clone(), loss_cfg, None, None);
    ...
    for (lo, hi) in [(0, SPLIT), (SPLIT, G)] {
        ...
        let micro_loss = batch_loss(p_micro, o_micro, loss_cfg, None, None);
...
fn naive_equal_weighting_is_not_equivalent() {
    ...
    let x = attributes();
    let o = observations();
    let pooled: f32 = batch_loss(predictions(&head, x.clone()), o.clone(), &cfg, None, None).into_scalar();
    let mut naive = 0.0f32;
    for (lo, hi) in [(0, SPLIT), (SPLIT, G)] {
        let p = predictions(&head, x.clone().slice([lo..hi, 0..F]));
        let l: f32 = batch_loss(p, o.clone().slice([lo..hi, 0..T]), &cfg, None, None).into_scalar();
        naive += 0.5 * l;
    }
