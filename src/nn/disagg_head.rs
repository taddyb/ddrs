...
impl DisaggHeadConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> DisaggHead<B> {
        let f = WINDOW + if self.use_attributes { self.num_attributes } else { 0 };
        ...
        DisaggHead { input, output, use_attributes: self.use_attributes }
    }
}
...
impl<B: Backend> DisaggHead<B> {
    ...
    /// `use_attributes` is false). The daily mean of each output day equals the
    /// corresponding `daily_q` value by construction.
    pub fn forward(&self, daily_q: Tensor<B, 2>, attrs: Tensor<B, 2>, n_hourly: usize) -> Tensor<B, 2> {
...
mod tests {
    ...
    fn mass_is_conserved_for_arbitrary_shape() {
        ...
        let hourly = head.forward(q.clone(), attrs, 72); // train trim: 3 days
    ...
    fn test_mode_uses_all_days_with_edge_clamp() {
        ...
        let hourly = head.forward(q.clone(), attrs, 72); // d_use = 3 == D
        for d in 0..3usize {
    ...
    fn default_init_is_nonflat_but_conserves_mass() {
        ...
        let hourly = head.forward(q.clone(), attrs, 72); // (72, 2)
    ...
    fn gradient_flows_to_output_layer() {
        ...
        let hourly = head.forward(q, attrs, 48); // (48, 2)
