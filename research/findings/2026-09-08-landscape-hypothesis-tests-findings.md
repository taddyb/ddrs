...
| NH hourly MTS-LSTM | 0.117 | 0 % | 0.042 | 3 % | +0.85 | 0.117 → 0.143 | 1 d early, router adds 1–2 |

The hourly MTS-LSTM read-out (2026-09-14) is the one arm where routing adds nothing: NSE 0.592 → 0.599, KGE
0.547 → 0.515. Against the retrospective control it loses 0.12 NSE at the median, 83 % of gauges move by more
than 0.05 and only 256 of those improve, and the loss is peak volume, not timing: median FHV −44 % against −18 %
for the control, KGE alpha (sigma_sim / sigma_obs) 0.65 against 0.91, and the routed→obs lag is 0 at 64 % of
gauges with the router matching the lag the gauge asks for at 61 %. The product's summed inflow already leads
observation by the same 1 day as the daily products at 3,000–30,000 km², so the timing story of the other four
arms holds here too; what the roughness law cannot repair is a product whose hourly peaks are too small.
rho(n_0, gamma) = +0.85 (the registered prediction held), gamma median 0.042 with 3 % at zero, and the n(d)
field barely breathes over WY2000 (network-median n moves 1.1 % between its highest and lowest day, per-reach
median 1.02x, p90 1.33x). Dam reaches sit 1–27 % *below* same-size unaffected reaches in n_0 on this arm
(Mann–Whitney p = 0.016, n = 2,178), the opposite sign to the retrospective arm's Midwest excess (§37.5), so the
dam signal is also product-specific. Read-out log:
`.ddrs/experiments/readout-2026-09-13T14-14-46Z-train-and-test.log`; plots under the run's `plots/`.
