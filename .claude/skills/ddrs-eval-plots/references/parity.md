# Reference: DDR ↔ DDRS parameter parity

Side-by-side distributions of the same learnable parameters (`n`, `q_spatial`,
`p_spatial`) produced by DDR-Python and by ddrs on the **same CONUS reaches**.
Two variants, same instruments:

| Section | Compares | When |
|---|---|---|
| §Init-time parity | freshly-initialized heads, no training | after touching `src/nn/`, the rskan pin, or DDR's `nn/kan.py` |
| §Trained parity | both codebases trained at `seed=42` | after a matched `train-and-test` pair |

Both use `scipy.stats.ks_2samp` (two-sample Kolmogorov–Smirnov) on the
distributions; trained parity adds `spearmanr` because at that point the
*per-reach* correspondence matters, not just the marginal.

Place the notebook at `<RUN_DIR>/plots/parity_{init,trained}.ipynb`. Init parity
is run-independent, but colocating keeps the artefacts with whichever run is
being investigated.
...
`scripts/dump_ddr_init_params.py`. The two NetCDFs they produce are the inputs
to this plot family.
...
KS < 0.05 on all three parameters. If `n` fails specifically, the saturation
symptom is a head-init divergence; if all three fail, look upstream
(denormalize, attribute normalisation). If all three pass yet the trained `n`
saturates after 5 epochs, the bug is in training dynamics (optimizer /
batching / loss) — outside the scope of the parity plan.

---

## Trained parity

Where init parity compares the INIT distributions, this compares the TRAINED
distributions on the exact same CONUS reaches.
...
| File | Producer |
|------|----------|
| `<RUN_DIR>/plot/kan_parameters.nc` | `cargo run --release --bin dump_parameters -- --config <RUN_DIR>/config.yaml --checkpoint <CKPT>/head --output <RUN_DIR>/plot/kan_parameters.nc` |
| `/tmp/kan_params_trained_ddr.nc` | `cd ~/projects/ddr && uv run python ~/projects/ddrs/scripts/dump_ddr_trained_params.py --checkpoint <DDR_CKPT> --conus-comids <RUN_DIR>/plot/kan_parameters.nc --out /tmp/kan_params_trained_ddr.nc` |

Runs made before the `plot/` convention wrote the dump to `<RUN_DIR>/kan_parameters.nc`
instead — check both. On a **leakance** run the run-root `kan_parameters.nc` is the
eval zeta diagnostic on dim `COMID_eval`, not the parameter dump; it has no `n`.
...
RUN_DIR  = Path("/home/tbindas/projects/ddrs/.ddrs/runs/<id>")
DDRS_NC  = RUN_DIR / "plot" / "kan_parameters.nc"
...
### Cell 4 — per-reach scatter (for `n`)
...
`KS ≤ 0.10` **and** `Spearman ≥ 0.90`. The `n` row of Cell 5 must print
`✓ same saturation` for the trained parity verdict to land. `q_spatial` and
`p_spatial` are reported for completeness — they were not the original symptom.

If `n` lands `~ borderline`: bring to user. If `n` lands `✗`: a new spec
localizing the `src/training/` divergence is warranted.
