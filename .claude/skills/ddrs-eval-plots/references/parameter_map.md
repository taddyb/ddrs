...
- **Use `pyogrio` column pushdown for the CONUS shapefile too, not just the global gpkg.** `pyogrio.read_dataframe(SHAPEFILE, columns=["COMID"])` reads the 2.2 GB `cat_pfaf_7` in **~5 s / 5.8 GB RSS** (346,327 polygons; measured 2026-07-31). `gpd.read_file` pulls every attribute column and is far slower for the same result.
- **`cat_pfaf_7_MERIT_Hydro_v07_Basins_v01_bugfix1.shp` ships WITHOUT a `.prj`** (unlike its `riv_pfaf_*` siblings), so `gpd.read_file`/`pyogrio` return `gdf.crs is None`. `contextily.add_basemap(crs=gdf.crs)` then fails or silently mis-registers tiles. Always `gdf = gdf.set_crs(epsg=4326)` when `crs is None` before plotting — MERIT is EPSG:4326.
- **Plot TWO colour scalings per variable when a run may be under-trained.** A head whose sigmoid output sits near 0.5 everywhere yields a parameter field spanning ~1–3% of the declared range; a single full-range map renders as flat colour and reads as "the model learned nothing," while a single percentile-stretched map hides the collapsed amplitude. Emitting both (`_fullrange` + `_stretched`, 2nd–98th pct) separates the two claims — the 2026-07-31 `aorc2f` run showed a *textbook* dendritic channel-network pattern in `n` at an amplitude of only 0.1312–0.1343 (2.6% of `[0.015, 0.25]`), which the full-range map alone would have missed entirely.
